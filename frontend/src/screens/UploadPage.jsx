...
const LocationPicker = dynamic(() => import("../components/LocationPicker.jsx"), { ssr: false, loading: () => <div className="map-loading">Konum seçici yükleniyor…</div> });
...
export default function UploadPage() {
  ...
  if (success) {
    return (
      <div className="page">
        ...
        <button className="btn btn--ghost" onClick={() => router.push("/harita")}>Haritaya Dön</button>
      </div>
    );
  }
  ...
  return (
    <div className="page">
      ...
      <h1 className="page-title--compact">Fotoğrafını Trabzon'un hafızasına ekle</h1>

      <ol className="upload-steps" aria-label="Yükleme adımları">
        {[
          "Fotoğraf",
          ...(needsLocation ? ["Konum", "Tarih"] : []),
          "Anı",
          "Bilgiler",
        ].map((step, index) => (
